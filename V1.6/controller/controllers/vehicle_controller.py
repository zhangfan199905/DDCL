...
_TRACI_LC_MODE = 0b00000000001 #0b000000000000
_TRACI_FORCED_LC_MODE = 0b000100000000
...
VARS_TO_SUBSCRIBE = [
    ...
    tc.VAR_LANE_ID, tc.VAR_LANE_INDEX, tc.VAR_LANEPOSITION, tc.VAR_TYPE, tc.VAR_ALLOWED_SPEED, tc.VAR_LEADER
]
...
class VehicleController:  
    def __init__(self, control_edge_ids: List[str]):
        ...
        # 使用独立的随机数生成器，避免跨实验的全局随机状态污染
        self._rng = random.Random(_PY_RANDOM_SEED)
        ...
        self.veh_type_cache: Dict[str, str] = {}
    ...
    def enforce_nocontrol_mode(self):
        new_vehicles = self._handle_subscriptions()
    ...
    def _handle_subscriptions(self) -> Set[str]:
        sim_results = traci.simulation.getSubscriptionResults()
        if not sim_results:
            return set()

        new_vehicles: Set[str] = set()
        ...
        for veh_id in sim_results.get(tc.VAR_DEPARTED_VEHICLES_IDS, []):
            if veh_id not in self.subscribed_vehicles:
                traci.vehicle.subscribe(veh_id, VARS_TO_SUBSCRIBE, parameters={tc.VAR_LEADER: 1000.0})
                #traci.vehicle.subscribeLeader(veh_id, 1000.0)
                self.subscribed_vehicles.add(veh_id)
                new_vehicles.add(veh_id)
    ...
    def update_vehicle_states(self, hml_lanes: Set[str], cdl_lanes: Set[str], ml_lanes: Set[str], cdl_start_edge: Optional[str], control_mode: int = MODE_CUSTOM):
        ...
        for veh_id in sorted(released_vehicles):
            if veh_id in self.all_veh_data:
                traci.vehicle.setLaneChangeMode(veh_id, _SUMO_DEFAULT_LC_MODE)

        # 2. 模式检查
        if control_mode != MODE_CUSTOM:
            for veh_id in sorted(current_controlled_veh_ids):
                traci.vehicle.setLaneChangeMode(veh_id, _SUMO_DEFAULT_LC_MODE)
        ...
        for veh_id in sorted(newly_controlled):
            traci.vehicle.setLaneChangeMode(veh_id, _TRACI_FORCED_LC_MODE)

        self.controlled_veh_ids = current_controlled_veh_ids

        # 4. 执行决策
        for veh_id in sorted(self.controlled_veh_ids):
    ...
    def manage_dcdl_lane_changing(self, veh_id: str, veh_data: Dict, hml_lanes: Set[str], cdl_lanes: Set[str], ml_lanes: Set[str], cdl_start_edge: Optional[str]):
        ...
        if coop_request < 1.0 and self._rng.random() >= prob_lc:
            return 
